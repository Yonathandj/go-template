...
COPY . .
ARG APP=api
# TARGETOS/TARGETARCH come from the requested image platform: cross-compile natively instead of emulating.
...
# Documentation only: the real port comes from server.port in the mounted config.
EXPOSE 8080
